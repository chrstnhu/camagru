...
const handlePagination = (postPerPage, currentPage) => {
  ...
  ul = document.querySelector(".pagination-list");
  if (!ul) {
  ...
  let li = "";
  li += `<li class="pagination-item pagination-item--prev ${currentPage <= 1 ? "pagination-item--hidden" : ""}" 
                onclick= "handlePagination(${postPerPage}, ${currentPage - 1})">
            <span class="pagination-icon"> &lt; </span></li>`;
  for (let pageNumber of pageNumbers) {
    if (pageNumber === "...") {
      li += `<li class="pagination-item pagination-item--ellipsis">...</li>`;
    } else {
      li += `<li class="pagination-item ${currentPage == pageNumber ? "pagination-item--active" : ""}" 
                   onclick= "handlePagination(${postPerPage}, ${pageNumber})">
                   ${pageNumber} 
                   </li>`;
    }
  }
  li += `<li class="pagination-item pagination-item--next ${currentPage >= totalPages ? "pagination-item--hidden" : ""}"
                onclick= "handlePagination(${postPerPage}, ${currentPage + 1})">
            <span class="pagination-icon"> &gt; </span></li>`;
